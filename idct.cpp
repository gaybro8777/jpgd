...
#define CLAMP(i) if (static_cast<uint>(i) > 255) i = (((~i) >> 31) & 0xFF)
/*----------------------------------------------------------------------------*/
...
struct Col
{
  static void idct(uint8* Pdst_ptr, const int* tempptr)
  {
    // will be optimized at compile time to either an array access, or 0
...
#define CLAMP2 CLAMP
    ...
    CLAMP2(i);
    Pdst_ptr[8*0] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp10 - btmp3, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*7] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp11 + btmp2, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*1] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp11 - btmp2, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*6] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp12 + btmp1, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*2] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp12 - btmp1, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*5] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp13 + btmp0, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*3] = (uint8)i;

    i = DESCALE_ZEROSHIFT(tmp13 - btmp0, CONST_BITS+PASS1_BITS+3);
    CLAMP2(i);
    Pdst_ptr[8*4] = (uint8)i;
  }
};
...
struct Col<1>
{
  static void idct(uint8* Pdst_ptr, const int* tempptr) 
  {
    ...
    CLAMP(dcval);
		const uint8 dcvalByte = static_cast<uint8>(dcval);
...
void idct(const jpgd_block_t* data, uint8* Pdst_ptr, int block_max_zag)
{
  ...
  const uint8* Prow_tab = &row_table[(block_max_zag - 1) * 8];
  int i;
